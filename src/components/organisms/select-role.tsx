...
const SelectRole: React.FC<Props> = () => {
  const projectKey: string | null = localStorage.getItem('PROJECT_KEY');
  const currentUser: any = useUser();
  const projects: any = useProjects();
  const setId: any = useDispatch();

  const filterDocument = (doc: { key: string }) => (projectKey ? doc.key === projectKey : null);
  ...
  const handlePick = (e: any) => {
    if (auth && auth.currentUser && firestore) {
      ...
      const dataToAdd = { user: { ...userToAdd, type: e.target.dataset.type } };
